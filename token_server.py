...
class TokenHandler(BaseHTTPRequestHandler):
    ...
    def _handle_search(self, qs: dict):
        ...
        ALLOWED_JURI = {"", "admin", "ce", "caa", "ta", "judic", "cass", "ca", "tj", "tcom", "constit", "europ", "cedh", "cjue", "doctrine", "legi"}
        ...
        _inconnues = [s for s in sources_only if s not in ("dila", "ariane", "admin", "cedh", "cjue", "doctrine", "legi")]
        if _inconnues:
            filtres_ignores.append({"parametre": "sources", "valeur": ",".join(_inconnues),
                                    "raison": "source(s) inconnue(s) (attendu : dila, ariane, admin, cedh, cjue, doctrine, legi) ; ignorée(s)"})
    ...
    def _handle_ssr_decision(self, source: str, decision_id: str):
        ...
        if source not in {"admin", "dila", "cedh", "cjue", "ariane", "cnil", "doctrine", "legi"}:
...
def _prechauffer_index() -> None:
    ...
    def _run():
        ...
        for q, juri in (("licenciement nul", "cassation"), ("permis de construire", None),
                        ...
                        ("responsabilité", "tj"), ("contrat", "tj"), ("contrat", "tcom"),
                        ("pourvoi", "cassation")):
            t0 = _tm.time()
            try:
